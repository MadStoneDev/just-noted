      ...
      <React.Fragment key={notebook.id}>
        ...
        >
          ...
          <div className="flex-1 min-w-0 py-1">
            ...
            >
              {effectivelyHidden && (
                <IconEyeOff size={12} className="text-[var(--color-text-tertiary)] flex-shrink-0" />
              )}
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-thin p-1.5">
      ...
      {hiddenNotebookCount > 0 && activeNotebookId === null && (
        <div className="px-2.5 py-1 flex items-center gap-1.5 text-[11px] text-[var(--color-text-tertiary)]">
          <IconEyeOff size={12} />
          <span>
            {hiddenNotebookCount} hidden notebook{hiddenNotebookCount !== 1 ? "s" : ""}
          </span>
        </div>
      )}
      ...
      <div className="h-px bg-[var(--color-border-secondary)] my-1.5" />

      {/* Create notebook */}
      <button
        onClick={onNewNotebook}
        disabled={notebookLimitReached}
        className={`w-full flex items-center gap-2 px-2.5 min-h-[44px] rounded-[var(--radius-md)] border border-dashed transition-colors ${
