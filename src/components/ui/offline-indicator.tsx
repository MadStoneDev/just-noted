...
export default function OfflineIndicator() {
  ...
  return (
    <>
      ...
      {showBanner && (
        <div
          className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full shadow-[var(--shadow-lg)] flex items-center gap-2 text-sm font-medium transition-all duration-300 ${
            isOnline
        ...
        >
          {isOnline ? (
            <>
              ...
              {pendingCount > 0 && (
                <span className="ml-1 px-2 py-0.5 bg-[var(--color-bg-primary)]/20 rounded-full text-xs">
          ...
          <button
            ...
            className="ml-2 p-1 hover:bg-[var(--color-bg-primary)]/20 rounded-full transition-colors"
          >
            <IconX size={14} />
          </button>
        </div>
      ...
      {!isOnline && !showBanner && (
        <button
          ...
          className="fixed top-4 right-4 z-50 p-2 bg-amber-500 text-amber-900 rounded-full shadow-[var(--shadow-lg)] hover:bg-amber-400 transition-colors"
